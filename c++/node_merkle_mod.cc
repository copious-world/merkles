...
namespace node_xxhash {
	...
	map<uint32_t,Merkle *> g_XXHASH_states;
	...
	NAN_METHOD(init) {
		...
		g_hash_count++;
		g_XXHASH_states[g_hash_count] = new Merkle(seed);
		//
	...
	NAN_METHOD(update) {
		...
		//
		Merkle *xxhash = g_XXHASH_states[key];
		if ( xxhash ) {
			//
	...
	NAN_METHOD(reset) {
		...
		if ( xxhash ) {
			...
			g_XXHASH_states[key] = new Merkle(seed);
			delete xxhash;
	...
	NAN_METHOD(hash_once_32) {
		...
		uint32_t seed = Nan::To<uint32_t>(info[1]).FromJust();
		//
		uint32_t hash = Merkle::hash(s.c_str(),s.size(),seed);
		//
